...
class AltPerformanceTest(TestTemplate):
    ...
    def run(self):
        for filename in self.input_files:
            self.write(filename)
            runtime = self.test_loop(filename)
...
class PerformanceTest1(TestTemplate):
    ...
    def __init__(self, input_folder, output_folder):
        ...
        self.input_params = self.gather_input_files(input_folder)
        self.output = {}

        self.total_iterations = 0
        for k, v in self.input_params.items():
            self.total_iterations += len(v)


    def gather_input_files(self, input_folder):
        file_list = os.listdir(input_folder)
        pattern = 'performanceTest1_(\d+)Requests_\d.json'
        input_params = {}
        for s in file_list:
            match = re.match(pattern, s)
            if match != None:
                filename = match.group(0)
                num_requests = match.group(1)
                if num_requests not in input_params:
                    input_params[num_requests] = []
                input_params[num_requests].append(filename)
        return input_params
    ...
    def run(self):
        count = 0
        for num_requests, file_list in self.input_params.items():
            self.output[num_requests] = []
            for filename in file_list:
                runtime = self.test_loop(filename)
                self.output[num_requests].append(runtime)
                count += 1
                self.write(count)
        self.save_results()
...
class PerformanceTest2(TestTemplate):
    ...
    def __init__(self, input_folder, output_folder):
        ...
        for k, v in self.input_params.items():
            for k2, v2 in v.items():
                self.total_iterations += len(v2)


    def gather_input_files(self, input_folder):
        file_list = os.listdir(input_folder)
        pattern = 'performanceTest2_(\d+)Telescopes_(\d+)Requests_\d.json'
        ...
        for s in file_list:
            ...
            if match != None:
                filename = match.group(0)
                num_telescopes = match.group(1)
                num_requests = match.group(2)
                if num_telescopes not in input_params:
                    input_params[num_telescopes] = {}
                if num_requests not in input_params[num_telescopes]:
                    input_params[num_telescopes][num_requests] = []
                input_params[num_telescopes][num_requests].append(filename)
        return input_params
    ...
    def run(self):
        count = 0
        for num_telescopes, v in self.input_params.items():
            self.output[num_telescopes] = {}
            for num_requests, file_list in v.items():
                self.output[num_telescopes][num_requests] = []
                for filename in file_list:
                    runtime = self.test_loop(filename)
                    self.output[num_telescopes][num_requests].append(runtime)
                    count += 1
                    self.write(count)
        self.save_results()
...
class PerformanceTest3(TestTemplate):
    ...
    def gather_input_files(self, input_folder):
        file_list = os.listdir(input_folder)
        pattern = 'performanceTest3_(\d+)SliceSize_\d+Requests_\d.json'
        ...
        for s in file_list:
            ...
            if match != None:
                filename = match.group(0)
                slice_size = match.group(1)
                if slice_size not in input_params:
                    input_params[slice_size] = []
                input_params[slice_size].append(filename)
        return input_params
    ...
    def run(self):
        count = 0
        for slice_size, file_list in self.input_params.items():
            self.output[slice_size] = []
            for filename in file_list:
                runtime = self.test_loop(filename)
                self.output[slice_size].append(runtime)
                count += 1
                self.write(count)
        self.save_results()
...
class PerformanceTest5(TestTemplate):
    ...
    def gather_input_files(self, input_folder):
        file_list = os.listdir(input_folder)
        pattern = 'performanceTest5_(\d+)Networks_\d+Telescopes_\d+RequestsPerNetwork_\d.json'
        ...
        for s in file_list:
            ...
            if match != None:
                filename = match.group(0)
                num_networks = match.group(1)
                if num_networks not in input_params:
                    input_params[num_networks] = []
                input_params[num_networks].append(filename)
        return input_params
    ...
    def run(self):
        count = 0
        for num_networks, file_list in self.input_params.items():
            self.output[num_networks] = []
            for filename in file_list:
                runtime = self.test_loop(filename)
                self.output[num_networks].append(runtime)
                count += 1
                self.write(count)
        self.save_results()
...
class PerformanceTest6(TestTemplate):
    """Test the effect of a varying number of requests on the performance of a scheduler with a single telescope."""
    def __init__(self, input_folder, output_folder):
        super().__init__()
        self.input_folder = input_folder
    ...
    def gather_input_files(self, input_folder):
        file_list = os.listdir(input_folder)
        pattern = 'performanceTest6_(\d+)Networks_(\d+)Telescopes_(\d+)GrossRequests_\d+.json'
